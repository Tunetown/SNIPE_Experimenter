...
import com.dkriesel.snipe.util.MersenneTwisterFast;
 ...
 * 
 * <li>[Igel2003] Igel and H�sken, Empirical evaluation of the improved Rprop
...
public class NeuralNetwork implements Serializable {
	...
	public NeuralNetwork(NeuralNetworkDescriptor descriptor) {
		int[] layers = descriptor.getNeuronsPerLayer().clone();
		...
		if (weight != null) {
			...
			if (Math.random() > 0.5) {
				setSynapse(
						...
						weight[1],
						originalWeight
								+ Math.exp(getRandomDoubleBetweenIncluding(-4,
										1)));
			} else {
				setSynapse(
						weight[0],
	...
	public void mutateWeightsGaussianAdaptivePertubationVectorDriven() {
		...
		// used.
		double[][] perturbationVector = shadows
				.get(SHADOWKEY_gaussianMutationAdaptivePertubationVector);

		// mutate perturbation vector
		for (int i = 0; i < predecessorWeights.length; i++) {
			for (int j = 0; j < predecessorWeights[i].length; j++) {
				perturbationVector[i][j] *= Math.exp(globalStandardDeviation
	...
	protected double getUserShadowValue(int i, int j, String key) {
		...
		double[][] shadow = shadows.get(USERSHADOWPREFIX + key);
		if (shadow == null) {
	...
	protected double[][] getUserShadow(String key) {
		return shadows.get(USERSHADOWPREFIX + key);
	}
			...
			for (int i = 0; i < countNeurons() + 1; i++) {
				for (int j = 0; j < predecessors[i].length; j++) {
					...
					if (Math.abs(decisionValue) < EPSILON) {
						...
						lastUpdates[i][j] = -(Math.signum(gradients[i][j]))
								* learningRates[i][j];
	...
	public int[] getSynapseRandomExistent() {
		...
		// ziehe so lange unterarrayl�ngen von der chosensynapse ab, bis
